...
async def test_non_negative_float_nok_mutation_ok():
    @Resolver("Mutation.nonNegativeFloat", schema_name="test_non_negative_float_mutation_ok")
    ...
    assert await engine.execute('mutation nonNegativeFloat { nonNegativeFloat(input:100) }') == {
...
async def test_non_negative_float_mutation_nok():
    ...
    result = await engine.execute('mutation nonNegativeFloat { nonNegativeFloat(input:-100) }')
    assert result['data'] is None
    assert len(result['errors']) == 1
    assert result['errors'][0]['message'] == 'Value -100 is not of correct type NonNegativeFloat'
