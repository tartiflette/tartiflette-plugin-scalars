...
async def test_hex_color_code_mutation_ok():
    @Resolver(
        "Mutation.hexColorCode", schema_name="test_hex_color_code_mutation_ok"
    )
    async def hex_color_code_resolver(*_args, **_kwargs):
        return True
    ...
    assert await engine.execute(
        'mutation hexColorCode { hexColorCode(input:"#ffffff") }'
...
async def test_hex_color_code_mutation_nok():
    @Resolver(
        "Mutation.hexColorCode", schema_name="test_hex_color_code_mutation_nok"
    )
    async def hex_color_code_resolver(*_args, **_kwargs):
        return True
    ...
    result = await engine.execute(
        'mutation hexColorCode { hexColorCode(input:"nok") }'
    )
    assert result["data"] is None
    assert len(result["errors"]) == 1
    assert (
        result["errors"][0]["message"]
        == "Value nok is not of correct type HexColorCode"
    )
